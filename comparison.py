...
# Neural Network Models for Other Algorithms
...
algorithms = ["Custom", "QMIX", "MAPPO", "IPPO"]
metrics = defaultdict(lambda: defaultdict(list))

# Custom Algorithm with Fixes
...
# QMIX, MAPPO, IPPO classes remain unchanged for brevity
class QMIX:
    def __init__(self):
        self.policies = [SimplePolicy(obs_dim, action_dim).to(device) for _ in range(num_agents)]
        self.values = [SimpleValue(obs_dim * num_agents).to(device) for _ in range(num_agents)]
        self.optimizers = [torch.optim.Adam(list(self.policies[i].parameters()) + list(self.values[i].parameters()), lr=initial_lr) for i in range(num_agents)]

    def act(self, obs, epsilon, episode):
        obs_array = np.array(obs, dtype=np.float32)
        obs_tensor = torch.tensor(obs_array, dtype=torch.float32).to(device)
        actions = []
        log_probs = []
        for i in range(num_agents):
            action_probs = self.policies[i](obs_tensor[i:i+1])
            action_dist = Categorical(probs=action_probs)
    ...
    def update(self, log_probs, reward, obs):
        ...
        policy_loss = -torch.stack(log_probs).mean() * reward_tensor
        value_loss = 0
        for i in range(num_agents):
            value = self.values[i](global_obs)
            value_loss += F.mse_loss(value, reward_tensor.unsqueeze(0))
...
class MAPPO:
    def __init__(self):
        self.policies = [SimplePolicy(obs_dim, action_dim).to(device) for _ in range(num_agents)]
        self.value = SimpleValue(obs_dim * num_agents).to(device)
        self.optimizers = [torch.optim.Adam(self.policies[i].parameters(), lr=initial_lr) for i in range(num_agents)]
        self.value_optimizer = torch.optim.Adam(self.value.parameters(), lr=initial_lr)
...
class IPPO:
    def __init__(self):
        self.policies = [SimplePolicy(obs_dim, action_dim).to(device) for _ in range(num_agents)]
        self.values = [SimpleValue(obs_dim).to(device) for _ in range(num_agents)]
        self.optimizers = [torch.optim.Adam(list(self.policies[i].parameters()) + list(self.values[i].parameters()), lr=initial_lr) for i in range(num_agents)]
    ...
    def update(self, log_probs, reward, obs):
        ...
        value_loss = 0
        for i in range(num_agents):
            value = self.values[i](obs_tensor[i:i+1])
            value_loss += F.mse_loss(value, reward_tensor.unsqueeze(0))
        loss = policy_loss + 0.1 * value_loss
        for i in range(num_agents):
            self.optimizers[i].zero_grad()
...
for algo_name in algorithms:
    ...
    for episode in range(num_episodes):
        ...
        current_epsilon = max(final_epsilon, initial_epsilon - (episode * epsilon_decay))
        for optimizer in (algo.optimizers if algo_name in ["QMIX", "MAPPO", "IPPO"] else [algo.optimizer, algo.critic_optimizer] if algo_name == "Custom" else [algo.optimizer]):
        ...
        for step in range(max_steps):
            ...
            # Pass landmarks_covered for Custom algorithm's adaptive exploration
            ...
            if algo_name == "Custom":
                ...
                actions, log_probs = algo.act(obs, current_epsilon, episode)
            ...
            # Estimate coverage
            episode_coverage += sum(1 for r in rewards.values() if r > 0.1)
        ...
        coverage_history.append(episode_coverage / max_steps / num_agents)
        ...
        # Print progress
        if (episode + 1) % 50 == 0:
            print(f"{algo_name} - Episode {episode + 1}/{num_episodes}, "
    ...
    # Store final metrics
    metrics[algo_name]["shaped_reward"] = rewards_history
    metrics[algo_name]["coverage"] = coverage_history
    metrics[algo_name]["collisions"] = collision_history
    metrics[algo_name]["raw_reward"] = raw_rewards_history
    metrics[algo_name]["shaped_reward_per_step"] = shaped_rewards_history
    metrics[algo_name]["agent_distance"] = agent_distances_history
    metrics[algo_name]["landmark_dist"] = landmark_dist_history
    metrics[algo_name]["landmarks_covered"] = landmarks_covered_history
...
plt.subplot(2, 2, 1)
for algo_name in algorithms:
    smoothed = np.convolve(metrics[algo_name]["shaped_reward"], np.ones(10)/10, mode='valid')
...
plt.subplot(2, 2, 2)
for algo_name in algorithms:
    smoothed = np.convolve(metrics[algo_name]["raw_reward"], np.ones(10)/10, mode='valid')
    plt.plot(smoothed, label=algo_name)
...
plt.subplot(2, 2, 3)
for algo_name in algorithms:
    smoothed = np.convolve(metrics[algo_name]["landmarks_covered"], np.ones(10)/10, mode='valid')
    plt.plot(smoothed, label=algo_name)
...
plt.subplot(2, 2, 4)
for algo_name in algorithms:
    smoothed = np.convolve(metrics[algo_name]["landmark_dist"], np.ones(10)/10, mode='valid')
    plt.plot(smoothed, label=algo_name)
...
for algo_name in algorithms:
    print(f"\n{algo_name}:")
    print(f"Average Shaped Reward: {np.mean(metrics[algo_name]['shaped_reward'][-50:]):.2f} ± {np.std(metrics[algo_name]['shaped_reward'][-50:]):.2f}")
    print(f"Average Landmark Coverage: {np.mean(metrics[algo_name]['coverage'][-50:]):.2f} ± {np.std(metrics[algo_name]['coverage'][-50:]):.2f}")
    print(f"Average Collisions (est): {np.mean(metrics[algo_name]['collisions'][-50:]):.2f} ± {np.std(metrics[algo_name]['collisions'][-50:]):.2f}")
    print(f"Average Raw Reward: {np.mean(metrics[algo_name]['raw_reward'][-50:]):.2f} ± {np.std(metrics[algo_name]['raw_reward'][-50:]):.2f}")
    print(f"Average Agent Distance: {np.mean(metrics[algo_name]['agent_distance'][-50:]):.2f} ± {np.std(metrics[algo_name]['agent_distance'][-50:]):.2f}")
    print(f"Average Min Landmark Distance: {np.mean(metrics[algo_name]['landmark_dist'][-50:]):.2f} ± {np.std(metrics[algo_name]['landmark_dist'][-50:]):.2f}")
    print(f"Average Landmarks Covered: {np.mean(metrics[algo_name]['landmarks_covered'][-50:]):.2f} ± {np.std(metrics[algo_name]['landmarks_covered'][-50:]):.2f}")